...
def get_currency_code(currency: AccountCurrencyEnum) -> str:
    currency_codes = {
        ...
        AccountCurrencyEnum.GHS: settings.CURRENCY_CODE_GHS,
    }
...
def generate_account_number(currency: AccountCurrencyEnum) -> str:
    try:
        ...
        logger.error(f"HTTP Exception in account number generation: {http_ex}")
        raise http_ex
        ...
        raise HTTPException(
            ...
            detail={
                "status": "error",
                "message": f"Failed to generate account numner: {e}"
            }
        )