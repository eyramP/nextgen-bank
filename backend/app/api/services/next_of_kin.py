...
async def get_user_nex_of_kins(
    user_id: UUID,
    ...
    try:
        ...
        logger.error(f"Failed to retreive next of kins: {str(e)}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def update_next_of_kin(
    user_id: UUID,
    next_of_king_id: UUID,
    update_data: NextOfKinUpdateSchema,
    session: AsyncSession
) -> NextOfKin:
    try:
        next_of_kin = await get_user_next_of_kin(user_id, next_of_king_id, session)

        if update_data.is_primary is not None:
            if update_data.is_primary:
                existing_primary = await get_primary_next_of_kin(user_id, session)
                if existing_primary and existing_primary.id != next_of_king_id:
                ...
                if total_count == 1:
                    raise HTTPException(
                        ...
                        detail={
                            "status": "error",
                            "message": "Cannot unset primary next of kin when there is only next of kin"
                        }
                    )
        ...
        await session.commit()
        await session.refresh(next_of_kin)
        logger.info(f"Updated next of kin: {next_of_kin} for user: {user_id}")
