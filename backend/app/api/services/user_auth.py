...
class UserAuthService:
    async def get_user_by_email(
        ...
        # session refers to the database session
    ...
    async def verify_user_password(self, plain_password: str, hashed_password: str) -> bool:
        return verify_passowrd(plain_password, hashed_password)
        ...
        try:
            ...
            for attempt in range(3):
                try:
                    ...
                    logger.info(f"OTP sent to user {user.email} successfuly")
                    return True, otp
                except Exception as e:
                    logger.error(f"Failed to send otp email {attempt +1}: {e}")
                    if attempt == 2:
                        user.otp = ""
                        user.otp_expiry_time = None
                        await session.commit()
    ...
    async def create_user(
        ...
        try:
            ...
            logger.error(f"Failed to send activation to {new_user.email}: {e}")
            raise
    ...
    async def verify_login_otp(
        ...
        try:
            ...
            logger.error(f"Error during OTP verificartion {e}")
            raise HTTPException(
    ...
    async def check_user_lockout(
        ...
        logger.warning(f"Attempted login to locked account by: {user.email}")
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail={
                "status": "error",
                "message": "Your account is temporarily locked",
                "action": f"Please try again after {remaining_minutes}",
    ...
    async def increment_failed_login_attempt(
        ...
        if user.failed_login_attempts >= settings.LOGIN_ATTEMPTS:
            ...
            try:
                ...
                logger.info(f"Account lockout notificaiton send to {user.email}")
            ...
            logger.warning(
                f"User {user.email} has been locked out dur to too many failed login attempts"
                )
        await session.commit()
        await session.refresh(user)
